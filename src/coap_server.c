...
static struct k_work send_dali_cmd;
...
void main(void)
{
	...
	k_work_init(&send_dali_cmd, do_send_dali_command);
	...
	ret = srp_init();
	if (ret) {
		LOG_ERR("Could not initialize OpenThread SRP");
		goto end;
	}
